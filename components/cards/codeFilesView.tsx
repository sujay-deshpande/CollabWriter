...
const getItemRoute = (item: CodeFileItem) => `/code-editor/codes/${item.id}`
...
const CodeFilesView = ({ items, userId }: { items: CodeFileItem[]; userId: string }) => {
  ...
  const handleCreateNewProject = () => {
    ...
    startTransition(async () => {
      ...
      router.push(`/code-editor/codes/${projectId}`)
    })
  }
    ...
    <section className='flex flex-col gap-5'>
      <div className='flex flex-wrap items-center justify-between gap-4'>
        <div className='flex flex-col gap-1'>
          <h1 className='text-20 font-bold text-white-1'>Your Code Projects</h1>
          <p className='text-12 text-white-4'>Browse your projects, shared work, and ownership status.</p>
        </div>
        ...
        <Button
          ...
          disabled={isPending}
        >
          {isPending ? 'Creating...' : 'Add new project'}
        </Button>
      </div>
            ...
            >
              <Link href={getItemRoute(item)} className='block'>
                <div className='mb-4 flex items-center justify-between gap-2'>
                  <span className='rounded-md bg-[#ffffff10] px-2 py-1 text-12 text-white-2'>Code</span>
                  <span
                    className={cn(
